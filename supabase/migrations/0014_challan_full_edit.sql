-- ============================================================================
-- Delivery challan: full edit of an Open challan (fields + materials, re-synced)
-- ============================================================================
-- update_challan_quantities (0012) only lets an Open challan's line QUANTITIES
-- change. This adds update_challan_full, which lets every field change on an
-- Open challan — company, date, job, reference, vehicle, notes AND the set of
-- materials (add / remove / swap) — re-syncing inventory atomically.
--
-- Strategy mirrors "cancel + recreate" without changing status: delete this
-- challan's existing dispatch issues (so material_balance shows the restored
-- balance), stock-check every new line, rewrite the header + lines, then re-post
-- one stock-out issue per line. dc_guard is told to permit the lines change via
-- the tx-local app.allow_dc_line_edit flag (same mechanism 0012 uses). Invoiced /
-- cancelled challans are rejected (they stay locked — cancel to reverse stock).
